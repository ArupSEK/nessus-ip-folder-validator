...
from nessus_fixed import NessusAPIError, NessusClient, build_index_csv_export, build_index_fast_api, make_scan_records, summarize_results, unix_from_date
...
st.caption('Validate Excel/CSV IP lists across Nessus scan folders, latest scan history, configured targets, host results, and authentication evidence.')
...
def to_excel_bytes(summary: pd.DataFrame, details: pd.DataFrame, auth: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for sheet_name, frame in (('Summary', summary), ('All_Matches', details), ('Auth_Evidence', auth)):
            ...
            for column_number, column_name in enumerate(frame.columns):
                ...
                if not frame.empty:
                    width = min(max(width, int(frame[column_name].astype(str).map(len).quantile(0.9)) + 2), 55)
                worksheet.set_column(column_number, column_number, width, wrap_format)
            worksheet.freeze_panes(1, 0)
            worksheet.autofilter(0, 0, max(len(frame), 1), max(len(frame.columns) - 1, 0))
    return output.getvalue()

def build_client() -> NessusClient:
    return NessusClient(base_url=st.session_state['base_url'], access_key=st.session_state['access_key'], secret_key=st.session_state['secret_key'], verify_ssl=st.session_state['verify_ssl'], timeout=int(st.session_state['timeout']))
with st.sidebar:
    ...
    mode = st.selectbox('Mode', ['Fast API mode', 'Fast API + host details', 'Reliable CSV export mode'], index=0, help='Fast mode now automatically opens host details when a host is returned only as a DNS name. CSV export remains best for exact Plugin Output.')
    include_history = st.checkbox('Include older scan histories', value=False, help='Disabled: check only the latest scan run. Enabled: also search older runs.')
    st.caption('Recommended: leave disabled so authentication is based only on the latest run.')
...
with main_left:
    ...
    if uploaded:
        try:
            ...
            ip_column = st.selectbox('Select IP column', input_df.columns, index=list(input_df.columns).index(auto_column))
            st.dataframe(input_df.head(10).copy(), use_container_width=True)
...
with main_right:
    ...
    st.info('An IP configured in a scan is no longer marked Not Found just because Nessus returned an empty host list or a hostname instead of the IP.')
...
if run:
    ...
    try:
        ...
        if mode == 'Reliable CSV export mode':
            ...
            matches, auth_rows = build_index_fast_api(client, input_ips, scan_records, include_history=include_history, fetch_host_details=mode == 'Fast API + host details', progress_callback=progress_callback)
...
if 'summary' in st.session_state:
    ...
    target_only = int((summary['Presence Type'] == 'Configured target').sum()) if total else 0
    ...
    download_csv.download_button('⬇️ Download Summary CSV', csv_bytes, file_name='nessus_ip_validation_summary.csv', mime='text/csv')
    download_excel.download_button('⬇️ Download Full Excel Report', excel_bytes, file_name='nessus_ip_validation_report.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
